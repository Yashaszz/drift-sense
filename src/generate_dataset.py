...
``stress`` covers the full 9:1 to 11:1 magnification band the problem statement
names for robustness testing (section 4A: "the sessions indicated robustness
testing may include approximately 9:1 to 11:1"). The shipped strata top out at
+-5%, so ``large`` reaches only 9.5:1 to 10.5:1 -- half the stated range. It is
deliberately **not** in :data:`DEFAULT_POSE_CONDITIONS`: adding it to the
shipped set would change every measured number, and the point of this stratum is
to answer a question about the shipped pipeline, not to replace it. Generate it
into its own directory with ``--pose-conditions stress``.
"""

DEFAULT_POSE_CONDITIONS: tuple[str, ...] = ("none", "small", "large")
"""Pose strata generated by default.

The shipped dataset's stratification. ``stress`` is excluded on purpose -- see
:data:`POSE_RANGES`.
"""
...
def build_dataset(
    ...
    *,
    ...
    seeds_per_cell
        ...
        ``2 * 2 * len(pose_conditions) * len(noise_levels) * seeds_per_cell``.
    base_seed
    ...
    pose_conditions
        Pose strata to generate; defaults to :data:`DEFAULT_POSE_CONDITIONS`.
    ...
    poses = tuple(DEFAULT_POSE_CONDITIONS if pose_conditions is None else pose_conditions)
    for pose_name in poses:
        if pose_name not in POSE_RANGES:
            msg = f"unknown pose condition {pose_name!r}; known strata are {tuple(POSE_RANGES)}"
            raise ValueError(msg)
    ...
    expected = expected_pair_count(seeds_per_cell, levels, poses)
...
def expected_pair_count(
    ...
    return 2 * 2 * len(poses) * len(levels) * seeds_per_cell
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "pose strata to generate; defaults to the shipped "
            f"{' '.join(DEFAULT_POSE_CONDITIONS)}. 'stress' covers the full "
            "9:1-11:1 magnification band the problem statement names for "
            "robustness testing, which the shipped strata do not reach. "
            "Generate it into its own --output-dir: it is a separate "
            "experiment, not a replacement for the measured set."
        ),
    )
    parser.add_argument(
        "--overlay-check",
    ...
    records, elapsed, gt_path = build_dataset(
        ...
        pose_conditions=args.pose_conditions,
    )
